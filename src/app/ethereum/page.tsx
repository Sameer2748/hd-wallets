...
// Mock toast for demonstration - replace with actual sonner
const toast = {
...
export default function EthereumWalletComponent() {
  ...
  useEffect(() => {
    const loadWalletsAndBalances = async () => {
      const localMnemonic = localStorage.getItem("eth-mnemonic");
      const localWallets = localStorage.getItem("eth-wallets");
      ...
      if (localWallets) {
        ...
        console.log(`Fetching balances for ${parsedWallets.length} Ethereum wallets...`);
  
        try {
          const balancePromises = parsedWallets.map(async (wallet, index) => {
            try {
              const res = await fetch(`/api/get-eth-balance?publicKey=${encodeURIComponent(wallet.publicKey)}`);
              if (!res.ok) {
              ...
              console.log(`Wallet ${index + 1}: ${balance} ETH`);
          ...
          localStorage.setItem("eth-wallets", JSON.stringify(walletsWithBalances));
  
          console.log("All Ethereum wallet balances fetched successfully!");
        } catch (error) {
  ...
  const fecthBalance = async (publicKey: string) => {
    const solanaResult = await fetchWalletBalance(
      'https://eth-mainnet.g.alchemy.com/v2/gN9fcg5Y_oVUVjoNZPYTewKErIQo-uwp',
      publicKey,
      'ethereum'
    );
  ...
  const createMnemonic = () => {
    let localMnemonic = localStorage.getItem("eth-mnemonic");
    if (localMnemonic) {
    ...
    if (newMnemonic) {
      ...
      localStorage.setItem("eth-mnemonic", newMnemonic);
  ...
  const deleteMnemonic = () => {
    ...
    setWallets([]);
    setCurrentIndex(0);
    localStorage.removeItem("eth-mnemonic");
  ...
  const createWallet = async () => {
    ...
    try {
      ...
      if (publicKeyString && privateKeyString) {
       ...
       // Inside createWallet() after adding new wallet
try {
  console.log(`Fetching balance for new wallet: ${publicKeyString}`);

  const res = await fetch(`/api/get-eth-balance?publicKey=${publicKeyString}`);
  const result = await res.json();

  const balance = result.success ? result.balance.toString() : '0';
  console.log(`New wallet balance: ${balance} ETH`);
  ...
  toast.success(`Balance loaded: ${balance} ETH`);
} catch (balanceError) {
      ...
      <div className="container mx-auto px-8 py-6 max-w-7xl">
        ...
        <div className="flex items-center justify-between mb-12">
          <div className="flex items-center gap-3">
            ...
            <Link href={"/"}>
              <span className="text-3xl font-bold">Ethereum Wallets</span>
            </Link>
          </div>
          <button
        ...
        {!mnemonic ? (
              ...
              <div className="max-w-2xl mx-auto space-y-6">
                <div>
                  ...
                  <textarea
                    ...
                    className={`w-full p-4 rounded-xl text-sm border-1 resize-none h-24 ${isDarkMode
                ...
                <button
                  onClick={() => {
                    ...
                    if (inputValue) {
                      ...
                      localStorage.setItem("eth-mnemonic", inputValue);
          ...
          <>
            ...
            <div className="mb-16">
              ...
              {wallets.length > 0 ? (
                <div
                  className={`grid gap-6 ${isGridView ? "grid-cols-1 lg:grid-cols-3" : "grid-cols-1"
                    }`}
                >
